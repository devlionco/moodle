 ...
 * External course API
 *
 * @package    block_sharedwithme_external
 * @category   external
...
require_once($CFG->dirroot . '/blocks/sharedwithme/lib.php');
 ...
 * @since Moodle 2.2
 */
class block_sharedwithme_external extends external_api {
    ...
    public static function render_courses_block($perpage) {
        ...
        $render = block_sharedwithme_render_courses_content($perpage);

        $data = new \StdClass();
        $data->content = $render;
        $data->content_empty = !empty($render) ? false : true;
        $data->pix_no_courses = $OUTPUT->image_url('courses', 'block_sharedwithme');
        $content = $OUTPUT->render_from_template('block_sharedwithme/content-courses', $data);
