...
/**
 * Indicatoressay question definition class.
 *
 * @package    qtype
...
/**
 * Represents an indicatoressay question.
 *
 * @copyright  2009 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_indicatoressay_question extends question_with_responses {
     ...
     * @return qtype_indicatoressay_format_renderer_base the response-format-specific renderer.
     */
    public function get_format_renderer(moodle_page $page) {
        return $page->get_renderer('qtype_indicatoressay', 'format_' . $this->responseformat);
    }
    ...
    public function summarise_response(array $response) {
        ...
        if (isset($response['attachments'])  && $response['attachments']) {
            ...
            if ($attachedfiles) {
                $output .= get_string('attachedfiles', 'qtype_indicatoressay', implode(', ', $attachedfiles));
            }
        }
        return $output;
    }
    ...
    public function check_file_access($qa, $options, $component, $filearea, $args, $forcedownload) {
        ...
        } else if ($component == 'qtype_indicatoressay' && $filearea == 'graderinfo') {
    ...
    private function check_input_word_count($responsestring) {
        ...
        if ($this->maxwordlimit && $count > $this->maxwordlimit) {
            return get_string('maxwordlimitboundary', 'qtype_indicatoressay',
                    ['limit' => $this->maxwordlimit, 'count' => $count]);
        } else if ($count < $this->minwordlimit) {
            return get_string('minwordlimitboundary', 'qtype_indicatoressay',
    ...
    public function get_word_count_message_for_review(array $response): string {
        ...
        if ($this->maxwordlimit && $count > $this->maxwordlimit) {
            return get_string('wordcounttoomuch', 'qtype_indicatoressay',
                    ['limit' => $this->maxwordlimit, 'count' => $count]);
        } else if ($count < $this->minwordlimit) {
            return get_string('wordcounttoofew', 'qtype_indicatoressay',
                    ['limit' => $this->minwordlimit, 'count' => $count]);
        } else {
            return get_string('wordcount', 'qtype_indicatoressay', $count);
        }
    }

    // For combinable indicatoressay type question.
