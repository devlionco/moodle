...
class qtype_combined_combinable_type_essay extends qtype_combined_combinable_type_base {
    ...
    protected $identifier = 'indicatoressay';
...
class qtype_combined_combinable_essay extends qtype_combined_combinable_text_entry {
    ...
    public function add_form_fragment(moodleform $combinedform, MoodleQuickForm $mform, $repeatenabled) {
        ...
        $mform->addElement('select', $this->form_field_name('responseformat'),
            get_string('responseformat', 'qtype_indicatoressay'), $formats);

        $responseformat = (!empty($CFG->qtype_indicatoressay_responseformat)) ? $CFG->qtype_indicatoressay_responseformat : 'editor';
        ...
        $mform->addElement('hidden', $this->form_field_name('attachments'),
            get_string('allowattachments', 'qtype_indicatoressay'));
        $mform->setType($this->form_field_name('attachments'), PARAM_INT);
        $mform->setDefault($this->form_field_name('attachments'), 0);

        // $attachments = (!empty($CFG->qtype_indicatoressay_attachments)) ? $CFG->qtype_indicatoressay_attachments : 0;
        ...
        // $mform->setDefault($this->form_field_name('attachmentsrequired'), 0);
        // $mform->addHelpButton($this->form_field_name('attachmentsrequired'), $this->form_field_name('attachmentsrequired'), 'qtype_indicatoressay');
        // $mform->disabledIf($this->form_field_name('attachmentsrequired'), $this->form_field_name('attachments'), 'eq', 0);

        // $mform->addElement('filetypes', $this->form_field_name('filetypeslist'), get_string('acceptedfiletypes', 'qtype_indicatoressay'));
        // $mform->addHelpButton($this->form_field_name('filetypeslist'), $this->form_field_name('acceptedfiletypes'), 'qtype_indicatoressay');
        // $mform->disabledIf($this->form_field_name('filetypeslist'), $this->form_field_name('attachments'), 'eq', 0);

        $mform->addElement('editor', $this->form_field_name('responsetemplate'), get_string('responsetemplate', 'qtype_indicatoressay'),
            array('rows' => 10), $combinedform->editoroptions);
        $mform->addHelpButton($this->form_field_name('responsetemplate'), 'responsetemplate', 'qtype_indicatoressay');

        $mform->addElement('editor', $this->form_field_name('graderinfo'), get_string('graderinfo', 'qtype_indicatoressay'),
            array('rows' => 10), $combinedform->editoroptions);
    }
