 ...
 * needed to restore one indicatoressay qtype plugin
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_qtype_indicatoressay_plugin extends restore_qtype_plugin {
    ...
    protected function define_question_plugin_structure() {
        return array(
            new restore_path_element('indicatoressay', $this->get_pathfor('/indicatoressay'))
        );
    }

    /**
     * Process the qtype/indicatoressay element
     */
    public function process_indicatoressay($data) {
        global $DB;
        ...
        // qtype_indicatoressay too.
        if ($questioncreated) {
            $data->questionid = $this->get_new_parentid('question');
            $newitemid = $DB->insert_record('qtype_indicatoressay_options', $data);

            // PTL_7328 backup & restore custom parameter (will work just on the same instance)
            $allowcheck = get_config('qtype_indicatoressay', 'allowcheck_' . $this->get_old_parentid('question'));
            if ($allowcheck) {
                set_config('allowcheck_' . $this->get_new_parentid('question'), $allowcheck, 'qtype_indicatoressay');
            }

            $this->set_mapping('qtype_indicatoressay', $oldid, $newitemid);
        }
    }
    ...
    public static function define_decode_contents() {
        return array(
            new restore_decode_content('qtype_indicatoressay_options', 'graderinfo', 'qtype_indicatoressay'),
        );
    }

    /**
     * When restoring old data, that does not have the indicatoressay options information
    ...
    protected function after_execute_question() {
        ...
        $essayswithoutoptions = $DB->get_records_sql("
                 ...
                 LEFT JOIN {qtype_indicatoressay_options} qeo ON qeo.questionid = q.id
                     WHERE q.qtype = ?
                       AND qeo.id IS NULL
                       AND bi.backupid = ?
                       AND bi.itemname = ?
                ", array('indicatoressay', $this->get_restoreid(), 'question_created'));
        ...
        foreach ($essayswithoutoptions as $q) {
            ...
            $DB->insert_record('qtype_indicatoressay_options', $defaultoptions);
        }
    }
}
