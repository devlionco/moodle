...
/**
 * Question type class for the indicatoressay question type.
 *
 * @package    qtype
...
/**
 * The indicatoressay question type.
 *
 * @copyright  2005 Mark Nielsen
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_indicatoressay extends question_type {
    ...
    public function get_question_options($question) {
        global $DB;
        $question->options = $DB->get_record('qtype_indicatoressay_options',
    ...
    public function save_question_options($formdata) {
        ...
        $options = $DB->get_record('qtype_indicatoressay_options', array('questionid' => $formdata->id));
        if (!$options) {
            $options = new stdClass();
            $options->questionid = $formdata->id;
            $options->id = $DB->insert_record('qtype_indicatoressay_options', $options);
        }
        ...
        $options->graderinfo = $this->import_or_save_files($formdata->graderinfo,
            $context, 'qtype_indicatoressay', 'graderinfo', $formdata->id);
        ...
        $isgradestypescalar = $formdata->weightstyle;
        $researchquestion = $formdata->researchquestion;

        // $indicatorsoptions = new stdClass();
        // $indicatorsoptions->isgradestypescalar = $isgradestypescalar;
        // $indicatorsoptions->indicatorlist = json_decode($formdata->questionindicatorfulltable);
        // $options->indicators = json_encode($indicatorsoptions);

        $indicatorsoptions = qtype_indicatoressay_prepare_ind_options_json($isgradestypescalar, $formdata->questionindicatorfulltable, $researchquestion);
        $options->indicators = $indicatorsoptions;

        // PTL_7328 Save Allowcheck option co config, w/o addind new field to 'qtype_indicatoressay_options'.
        set_config('allowcheck_' . $formdata->id, $formdata->allowcheck, 'qtype_indicatoressay');

        $DB->update_record('qtype_indicatoressay_options', $options);
    }
    ...
    public function delete_question($questionid, $contextid) {
        ...
        $DB->delete_records('qtype_indicatoressay_options', array('questionid' => $questionid));
    ...
    public function response_formats() {
        return array(
            'editor' => get_string('formateditor', 'qtype_indicatoressay'),
            'editorfilepicker' => get_string('formateditorfilepicker', 'qtype_indicatoressay'),
            'plain' => get_string('formatplain', 'qtype_indicatoressay'),
            'monospaced' => get_string('formatmonospaced', 'qtype_indicatoressay'),
            'noinline' => get_string('formatnoinline', 'qtype_indicatoressay'),
        );
    }
    ...
    public function response_required_options() {
        return array(
            1 => get_string('responseisrequired', 'qtype_indicatoressay'),
            0 => get_string('responsenotrequired', 'qtype_indicatoressay'),
        );
    }
    ...
    public function response_sizes() {
        $choices = [
            2 => get_string('nlines', 'qtype_indicatoressay', 2),
            3 => get_string('nlines', 'qtype_indicatoressay', 3),
        ];
        for ($lines = 5; $lines <= 40; $lines += 5) {
            $choices[$lines] = get_string('nlines', 'qtype_indicatoressay', $lines);
        }
        return $choices;
    }
    ...
    public function attachments_required_options() {
        return array(
            0 => get_string('attachmentsoptional', 'qtype_indicatoressay'),
            1 => '1',
            2 => '2',
            3 => '3',
        );
    ...
    public function move_files($questionid, $oldcontextid, $newcontextid) {
        ...
        $fs->move_area_files_to_new_context($oldcontextid,
            $newcontextid, 'qtype_indicatoressay', 'graderinfo', $questionid);
    }

    protected function delete_files($questionid, $contextid) {
        parent::delete_files($questionid, $contextid);
        $fs = get_file_storage();
        $fs->delete_area_files($contextid, 'qtype_indicatoressay', 'graderinfo', $questionid);
    }
}
