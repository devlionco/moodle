...
namespace qtype_indicatoressay\form;
...
require_once($CFG->dirroot . '/question/type/indicatoressay/edit_essay_form.php');

/**
 * Unit tests for the indicatoressay edit form.
 *
...
class edit_form_test extends \advanced_testcase {
    ...
    protected function get_form($classname) {
        ...
        $fakequestion->qtype = 'indicatoressay';
    ...
    public function test_attachments_validation(int $allowed, int $required, array $expected): void {
        list($form, $category) = $this->get_form('qtype_indicatoressay_edit_form');
        $submitteddata = [
    ...
    public function user_preference_provider(): array {
        $valid = [];
        $invalid = ['attachmentsrequired' => get_string('mustrequirefewer', 'qtype_indicatoressay')];
        return [
