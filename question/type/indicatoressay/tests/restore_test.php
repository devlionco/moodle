...
class restore_test extends \restore_date_testcase {
    ...
    /**
     * Test missing qtype_indicatoressay_options creation.
     *
     * Old backup files may contain essays with no qtype_indicatoressay_options record.
     * During restore, we add default options for any questions like that.
     * That is what is tested in this file.
     */
    public function test_restore_create_missing_qtype_indicatoressay_options() {
        global $DB;

        // Create a course with one indicatoressay question in its question bank.
        ...
        $indicatoressay = $questiongenerator->create_question('indicatoressay', null, array('category' => $category->id));

        // Remove the options record, which means that the backup will look like a backup made in an old Moodle.
        $DB->delete_records('qtype_indicatoressay_options', ['questionid' => $indicatoressay->id]);
        ...
        $newessay = $DB->get_record_sql('SELECT q.*
                                             ...
                                             WHERE qbe.questioncategoryid = ?
                                               AND q.qtype = ?', [$newcategory->id, 'indicatoressay']);
        $this->assertTrue($DB->record_exists('qtype_indicatoressay_options', ['questionid' => $newessay->id]));
    }
}
