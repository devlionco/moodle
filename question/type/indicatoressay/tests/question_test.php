...
namespace qtype_indicatoressay;
...
class question_test extends \advanced_testcase {
    public function test_get_question_summary() {
        $indicatoressay = \test_question_maker::make_an_essay_question();
        $indicatoressay->questiontext = 'Hello <img src="http://example.com/globe.png" alt="world" />';
        $this->assertEquals('Hello [world]', $indicatoressay->get_question_summary());
    }
    ...
    public function test_summarise_response(int $responserequired, int $attachmentsrequired,
        ...
        // Create the indicatoressay question under test.
        $indicatoressay = \test_question_maker::make_an_essay_question();
        $indicatoressay->start_attempt(new question_attempt_step(), 1);

        $indicatoressay->responseformat = 'editor';
        $indicatoressay->responserequired = $responserequired;
        $indicatoressay->attachmentsrequired = $attachmentsrequired;
        ...
        $this->assertEquals($expected, $indicatoressay->summarise_response(
    ...
    public function summarise_response_provider(): array {
        return [
            'text input required, not attachments required'  =>
                [1, 0, 'This is the text input for this indicatoressay.', 0, 'This is the text input for this indicatoressay.'],
            'Text input required, one attachments required, one uploaded'  =>
                [1, 1, 'This is the text input for this indicatoressay.', 1, 'This is the text input for this indicatoressay.Attachments: 0 (1 bytes)'],
    ...
    public function test_is_same_response() {
        ...
        $indicatoressay->responsetemplate = '';

        $indicatoressay->start_attempt(new question_attempt_step(), 1);

        $this->assertTrue($indicatoressay->is_same_response(
                array(),
                array('answer' => '')));
        ...
        $this->assertFalse($indicatoressay->is_same_response(
                array('answer' => 'Hello'),
                array()));
    ...
    public function test_is_same_response_with_template() {
        ...
        $indicatoressay->responsetemplate = 'Once upon a time';
    ...
    public function test_is_complete_response() {
        ...
        $indicatoressay->responserequired = 1;
        $indicatoressay->attachmentsrequired = 0;
        $indicatoressay->responseformat = 'editor';

        // The empty string should be considered an incomplete response, as should a lack of a response.
        $this->assertFalse($indicatoressay->is_complete_response(array('answer' => '')));
        $this->assertFalse($indicatoressay->is_complete_response(array()));

        // Any nonempty string should be considered a complete response.
        $this->assertTrue($indicatoressay->is_complete_response(array('answer' => 'A student response.')));
        $this->assertTrue($indicatoressay->is_complete_response(array('answer' => '0 times.')));
        $this->assertTrue($indicatoressay->is_complete_response(array('answer' => '0')));

        // Test case for minimum and/or maximum word limit.
        $response = [];
        $response['answer'] = 'In this indicatoressay, I will be testing a function called check_input_word_count().';

        $indicatoressay->minwordlimit = 50; // The answer is shorter than the required minimum word limit.
        $this->assertFalse($indicatoressay->is_complete_response($response));

        $indicatoressay->minwordlimit = 10; // The  word count  meets the required minimum word limit.
        $this->assertTrue($indicatoressay->is_complete_response($response));

        // The word count meets the required minimum  and maximum word limit.
        $indicatoressay->minwordlimit = 10;
        $indicatoressay->maxwordlimit = 15;
        $this->assertTrue($indicatoressay->is_complete_response($response));

        // Unset the minwordlimit/maxwordlimit variables to avoid the extra check in is_complete_response() for further tests.
        $indicatoressay->minwordlimit = null;
        $indicatoressay->maxwordlimit = null;

        // Test the case where two files are required.
        $indicatoressay->attachmentsrequired = 2;

        // Attaching less than two files should result in an incomplete response.
        $this->assertFalse($indicatoressay->is_complete_response(array('answer' => 'A')));
        $this->assertFalse($indicatoressay->is_complete_response(
        ...
        $this->assertTrue($indicatoressay->is_complete_response(
        ...
        $indicatoressay->responserequired = 0;
        ...
        $indicatoressay->reponserequired = 0;
        $indicatoressay->responseformat = 'noinline';
        $indicatoressay->attachmensrequired = 1;
        ...
        $indicatoressay->reponserequired = 1;
        $this->assertTrue($indicatoressay->is_complete_response(
                array('attachments' => $attachments[1])));
    }
    ...
    public function test_get_question_definition_for_external_rendering() {
        ...
        $indicatoressay = \test_question_maker::make_an_essay_question();
        $indicatoressay->minwordlimit = 15;
        $indicatoressay->start_attempt(new question_attempt_step(), 1);
        $qa = \test_question_maker::get_a_qa($indicatoressay);
        $displayoptions = new question_display_options();

        $options = $indicatoressay->get_question_definition_for_external_rendering($qa, $displayoptions);
        ...
        $this->assertEquals($indicatoressay->minwordlimit, $options['minwordlimit']);
    ...
    public function get_min_max_wordlimit_test_cases(): array {
        return [
            ...
            'text input required, min word limit not reached'  => [1, 15, 25,
                get_string('minwordlimitboundary', 'qtype_indicatoressay', ['count' => 14, 'limit' => 15])],
            'text input required, max word limit is exceeded'  => [1, 5, 12,
                get_string('maxwordlimitboundary', 'qtype_indicatoressay', ['count' => 14, 'limit' => 12])],
    ...
    public function get_word_count_message_for_review_test_cases() {
        return [
            ...
            'min and max, answer within range' =>
                    [10, 25, get_string('wordcount', 'qtype_indicatoressay', 14)],
            'min and max, answer too short' =>
                    [15, 25, get_string('wordcounttoofew', 'qtype_indicatoressay', ['count' => 14, 'limit' => 15])],
            'min and max, answer too long' =>
                    [5, 12, get_string('wordcounttoomuch', 'qtype_indicatoressay', ['count' => 14, 'limit' => 12])],
            'min only, answer within range' =>
                    [14, null, get_string('wordcount', 'qtype_indicatoressay', 14)],
            'min only, answer too short' =>
                    [15, null, get_string('wordcounttoofew', 'qtype_indicatoressay', ['count' => 14, 'limit' => 15])],
            'max only, answer within range' =>
                    [null, 14, get_string('wordcount', 'qtype_indicatoressay', 14)],
            'max only, answer too short' =>
                    [null, 13, get_string('wordcounttoomuch', 'qtype_indicatoressay', ['count' => 14, 'limit' => 13])],
        ];
    }
    ...
    private function create_user_and_sample_attachments($numberofattachments = 4) {
        ...
        $helper = \test_question_maker::get_test_helper('indicatoressay');
        $attachments = [];
