...
/**
 * Indicatoressay question renderer class.
 *
 * @package    qtype
...
/**
 * Generates the output for indicatoressay questions.
 *
 * @copyright  2009 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_indicatoressay_renderer extends qtype_renderer {
    public function formulation_and_controls(question_attempt $qa,
        question_display_options $options) {
        global $CFG, $PAGE;
        $question = $qa->get_question();

        /** @var qtype_indicatoressay_format_renderer_base $responseoutput */
        ...
        $result .= html_writer::tag('div', get_string('studentanswer', 'qtype_indicatoressay'), array('class' => 'answer-label'));
        ...
        $allowcheck = get_config('qtype_indicatoressay', 'allowcheck_' . $qa->get_question()->id);
        if ($allowcheck && self::is_interactive($qa)) {
            ...
            if ($attempt) {
                ...
                if ($allowcheck && !$quizattempt->is_finished()) {
                    ...
                    $PAGE->requires->js_call_amd('qtype_indicatoressay/main', 'init', $jsparams);
                }
            } else {
                $result .= html_writer::tag('div', $check, array('class' => 'im-controls'));
                $PAGE->requires->js_call_amd('qtype_indicatoressay/main', 'init', $jsparams);
    ...
    public function files_read_only(question_attempt $qa, question_display_options $options) {
        ...
        $fileslabel = $options->add_question_identifier_to_label(get_string('answerfiles', 'qtype_indicatoressay'));
    ...
    public function files_input(question_attempt $qa, $numallowed,
        ...
        if (!empty($qa->get_question()->filetypeslist)) {
            $text = html_writer::tag('p', get_string('acceptedfiletypes', 'qtype_indicatoressay'));
    ...
    public function manual_comment(question_attempt $qa, question_display_options $options) {
        ...
        $isstudent = qtype_indicatoressay_is_student();
        $review = false;
        if (in_array($PAGE->pagetype, ['mod-quiz-review'])) {
            $review = true;
        }
        // if (($options->manualcomment != question_display_options::EDITABLE || $isstudent) && !$review) {
        //     return '';
        // }

        $question = $qa->get_question();
        $ind = $DB->get_record('qtype_indicatoressay_options',
            array('questionid' => $question->id), 'indicators');
        $qaid = $this->getProtectedValue($qa, 'id');

        if ($data = qtype_indicatoressay_parse_ind_options_json($ind->indicators)) {
            ...
            foreach ($data->indicatorlist as $key => $value) {
                ...
                if ($existresponse = $DB->get_record('qtype_indicatoressay_resp', ['questionattemptid' => $qaid, 'qindid' => $key])) {
                    $resp = $existresponse;
                ...
                $value->type = $value->type ? get_string($value->type, 'qtype_indicatoressay') : '';

                $data->indicatorlist[$key] = $value;
            }
            $data->indicators = array_values($data->indicatorlist);
            $data->qaid = $qaid;
            $data->maxmark = $this->getProtectedValue($qa, 'maxmark');
            $data->minfraction = $this->getProtectedValue($qa, 'minfraction');
            $data->maxfraction = $this->getProtectedValue($qa, 'maxfraction');
            $data->usageid = $this->getProtectedValue($qa, 'usageid');
            $data->slot = $this->getProtectedValue($qa, 'slot');
            $data->isstudent = $isstudent;

            if (($atleastoneindicator && $isstudent) || (!$review && !$isstudent)) {
                $output = $OUTPUT->render_from_template('qtype_indicatoressay/grade_indicators', $data);
            }

            if (!$isstudent) {
                $PAGE->requires->js_call_amd('qtype_indicatoressay/grade', 'init', ['qaid' => $data->qaid, 'data' => $data]);
            }
        }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qtype_indicatoressay_format_renderer_base extends plugin_renderer_base {
...
/**
 * An indicatoressay format renderer for essays where the student should not enter
 * any inline response.
 *
 * @copyright  2013 Binghamton University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_indicatoressay_format_noinline_renderer extends qtype_indicatoressay_format_renderer_base {

    protected function class_name() {
        return 'qtype_indicatoressay_noinline';
    }
...
/**
 * An indicatoressay format renderer for essays where the student should use the HTML
 * editor without the file picker.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_indicatoressay_format_editor_renderer extends qtype_indicatoressay_format_renderer_base {
    protected function class_name() {
        return 'qtype_indicatoressay_editor';
    }

    public function response_area_read_only($name, $qa, $step, $lines, $context) {
        return html_writer::tag('div', $this->prepare_response($name, $qa, $step, $context),
            ['class' => $this->class_name() . ' qtype_indicatoressay_response readonly',
    ...
    public function response_area_input($name, $qa, $step, $lines, $context) {
        ...
        $responselabel = $this->displayoptions->add_question_identifier_to_label(get_string('answertext', 'qtype_indicatoressay'));
        $output = html_writer::tag('label', $responselabel, [
            'class' => 'sr-only',
            'for' => $id,
        ]);
        $output .= html_writer::start_tag('div', array('class' =>
            $this->class_name() . ' qtype_indicatoressay_response'));
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_indicatoressay_format_editorfilepicker_renderer extends qtype_indicatoressay_format_editor_renderer {
    protected function class_name() {
        return 'qtype_indicatoressay_editorfilepicker';
    }
    ...
    protected function specific_filepicker_options($acceptedtypes, $draftitemid, $context) {
        debugging('qtype_indicatoressay_format_editorfilepicker_renderer::specific_filepicker_options() is deprecated, ' .
...
/**
 * An indicatoressay format renderer for essays where the student should use a plain
 * input box, but with a normal, proportional font.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_indicatoressay_format_plain_renderer extends qtype_indicatoressay_format_renderer_base {
    /**
     * @return string the HTML for the textarea.
     */
    protected function textarea($response, $lines, $attributes) {
        $attributes['class'] = $this->class_name() . ' qtype_indicatoressay_response form-control';
    ...
    protected function class_name() {
        return 'qtype_indicatoressay_plain';
    }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_indicatoressay_format_monospaced_renderer extends qtype_indicatoressay_format_plain_renderer {
    protected function class_name() {
        return 'qtype_indicatoressay_monospaced';
    }
}
