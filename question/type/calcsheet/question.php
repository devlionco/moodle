 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_calcsheet_question extends question_graded_by_strategy
    implements question_response_answer_comparer
{
    ...
    public function get_validation_error(array $response)
    {
        ...
        return get_string('pleaseenterananswer', 'qtype_calcsheet');
    }
    ...
    public static function compare_string_with_wildcard($string, $pattern, $solovalor)
    {
        ...
        $value = 0;
        $arr = array();
        foreach ($arr_json as $indsheet => $sheet) {

            foreach ($sheet->rows as $indiceF => $fila) {
                ...
                foreach ($fila->columns as $col => $columna) {
                    ...
                    if (empty($vars)) {
                        ...
                        $arr[$count_compare]['she'] = $indsheet;
        ...
        if (!empty($arr)) {
            foreach ($arr as $id => $cell) {
                ...
                if ($enable_form == 1) {
                    if (isset($arr_json2[$cell['she']]->rows[$cell['Fil']]->columns[$cell['Col']]->value)) {  //existe valor en la celda a evaluar?
                        if ($arr_json2[$cell['she']]->rows[$cell['Fil']]->columns[$cell['Col']]->value == $cell['Val']) { //el valor de las celdas es igual?
                            if (isset($arr_json2[$cell['she']]->rows[$cell['Fil']]->columns[$cell['Col']]->formula)) {
                                if (strcasecmp(trim($arr_json2[$cell['she']]->rows[$cell['Fil']]->columns[$cell['Col']]->formula), trim($cell['Form'])) == 0) {
                                    $aciValor++;
                                    $aciFormulas++;
                                }
                            } else {
                    ...
                    if (isset($arr_json2[$cell['she']]->rows[$cell['Fil']]->columns[$cell['Col']]->formula)) {
                        ...
                        $string = $arr_json2[$cell['she']]->rows[$cell['Fil']]->columns[$cell['Col']]->formula;
