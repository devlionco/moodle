...
/**
 * Checks file access for formulas questions.
 *
 * @package  qtype_formulas
 * @category files
 ...
 * @return array
 */
function qtype_formulas_get_units_array() {

    $arr = qtype_formulas_prepare_units();
...
function qtype_formulas_get_units_row_array($danounit) {
    global $CFG, $DB;
...
//    $tolerance = 0.01;
//
//var_dump(qtype_formulas_check_for_penalty($dano, $answer, $tolerance));exit;
...
function qtype_formulas_check_for_penalty($dano, $answer, $tolerance) {
    ...
    //Compare Unit
    $arr_units = qtype_formulas_get_units_row_array($dano['unit']);
    ...
    foreach($arr_coeff_units_new as $item){
        ...
        if(qtype_formulas_compare_with_tolerance($item, $answer['value'], $coefftolerance)){
            $value_validation = 1;
        }
    }
    ...
    $wrongvaluepenalty = qtype_formulas_get_wrongvaluepenalty();
    $wrongunitpenalty = qtype_formulas_get_wrongunitpenalty();
...
function qtype_formulas_compare_answer($dano, $answer, $tolerance) {
    global $CFG, $DB;
    ...
    if($dano['unit'] == $answer['unit']){
        return qtype_formulas_compare_with_tolerance($dano['value'], $answer['value'], $tolerance);
    }
...
function qtype_formulas_compare_with_tolerance($dano, $answer, $tolerance) {
    if($tolerance != 0){
        $left = $dano - $tolerance;
...
function qtype_formulas_prepare_units() {
    global $CFG, $DB;

    $sql = "SELECT * FROM {config} WHERE name='qtype_formulas_units'";
...
function qtype_formulas_get_wrongvaluepenalty() {
    global $CFG, $DB;

    $sql = "SELECT * FROM {config} WHERE name='qtype_formulas_wrongvaluepenalty'";
    $config = $DB->get_record_sql($sql);
...
function qtype_formulas_get_wrongunitpenalty() {
    global $CFG, $DB;

    $sql = "SELECT * FROM {config} WHERE name='qtype_formulas_wrongunitpenalty'";
    $config = $DB->get_record_sql($sql);
...
function qtype_formulas_prepare_units_for_student(qtype_formulas_question $question) {
    global $CFG, $DB;

    $arr_units = array();
    foreach($question->answers as $answer){ $arr_units[] = $answer->unit; }

    $all_units = qtype_formulas_prepare_units();
