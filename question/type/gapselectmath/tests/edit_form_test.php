...
require_once($CFG->dirroot . '/question/type/gapselectmath/edit_gapselectmath_form.php');


/**
 * Subclass of qtype_gapselectmath_edit_form_base that is easier to used in unit tests.
 *
 * @copyright 2012 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_gapselectmath_edit_form_base_testable extends qtype_gapselectmath_edit_form_base {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_gapselectmath_edit_form_test extends advanced_testcase {
    ...
    protected function get_form($classname) {
        ...
        $fakequestion->qtype = 'gapselectmath'; // Does not actually matter if this is wrong.
    ...
    public function test_get_illegal_tag_error() {
        list($form) = $this->get_form('qtype_gapselectmath_edit_form_base_testable');
        ...
        $this->assertEquals(get_string('tagsnotallowed', 'qtype_gapselectmath', $a), $form->get_illegal_tag_error('<ijk>'));

        $a->tag = '&lt;/cat&gt;';
        $this->assertEquals(get_string('tagsnotallowed', 'qtype_gapselectmath', $a), $form->get_illegal_tag_error('</cat>'));

        $a->tag = '&lt;br /&gt;';
        $this->assertEquals(get_string('tagsnotallowed', 'qtype_gapselectmath', $a), $form->get_illegal_tag_error('<i><br /></i>'));
        ...
        $a->tag = '&lt;i&gt;';
        $this->assertEquals(get_string('tagsnotallowedatall', 'qtype_gapselectmath', $a),
    ...
    public function test_number_of_choice_groups() {
        list($form) = $this->get_form('qtype_gapselectmath_edit_form');
        // Use reflection to get the protected property we need.
        $property = new ReflectionProperty('qtype_gapselectmath_edit_form', '_form');
    ...
    public function test_choices_validation() {
        list($form, $category) = $this->get_form('qtype_gapselectmath_edit_form');
