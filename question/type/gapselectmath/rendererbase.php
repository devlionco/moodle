 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qtype_gapselectmath_elements_embedded_in_question_text_renderer
    ...
    public function correct_response(question_attempt $qa) {
        ...
        foreach ($question->textfragments as $i => $fragment) {
            if ($i > 0) {
                ...
                $correctanswer .= $mathlive->static_formula($choice->text);
            }
            $correctanswer .= $fragment;
        }

        if (!empty($correctanswer)) {
            return get_string('correctansweris', 'qtype_gapselectmath',
