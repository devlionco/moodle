 ...
 * needed to restore one opensheet qtype plugin
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_qtype_opensheet_plugin extends restore_qtype_plugin {
    ...
    protected function define_question_plugin_structure() {
        return array(
            new restore_path_element('opensheet', $this->get_pathfor('/opensheet'))
        );
    }

    /**
     * Process the qtype/opensheet element
     */
    public function process_opensheet($data) {
        global $DB;
        ...
        if ($questioncreated) {
            ...
            $newitemid = $DB->insert_record('qtype_opensheet_options', $data);
            $this->set_mapping('qtype_opensheet', $oldid, $newitemid);
        }
    }
    ...
    public static function define_decode_contents() {
        return array(
            new restore_decode_content('qtype_opensheet_options', 'graderinfo', 'qtype_opensheet'),
        );
    }

    /**
     * When restoring old data, that does not have the opensheet options information
     * in the XML, supply defaults.
     */
    protected function after_execute_question() {
        global $DB;

        $opensheetswithoutoptions = $DB->get_records_sql("
                    SELECT q.*
                      FROM {question} q
                      JOIN {backup_ids_temp} bi ON bi.newitemid = q.id
                 LEFT JOIN {qtype_opensheet_options} qeo ON qeo.questionid = q.id
                     WHERE q.qtype = ?
                       AND qeo.id IS NULL
                       AND bi.backupid = ?
                       AND bi.itemname = ?
                ", array('opensheet', $this->get_restoreid(), 'question_created'));

        foreach ($opensheetswithoutoptions as $q) {
            ...
            $DB->insert_record('qtype_opensheet_options', $defaultoptions);
        }
    }
}
