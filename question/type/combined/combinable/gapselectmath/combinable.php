...
/**
 * Defines the hooks necessary to make the gapselectmath question type combinable
 *
...
class qtype_combined_combinable_type_gapselectmath extends qtype_combined_combinable_type_base {

    protected $identifier = 'selectmenuSci';
...
class qtype_combined_combinable_gapselectmath extends qtype_combined_combinable_accepts_numerical_param {
    ...
    public function add_form_fragment(moodleform $combinedform, MoodleQuickForm $mform, $repeatenabled) {
        $mform->addElement('advcheckbox', $this->form_field_name('shuffleanswers'), get_string('shuffle', 'qtype_gapselectmath'));

        $answerel = array($mform->createElement('mathlive',
                                                $this->form_field_name('answer'),
                                                get_string('choicex', 'qtype_gapselectmath'),
        ...
        $combinedform->repeat_elements($answerel,
                                        ...
                                        QUESTION_NUMANS_ADD,
                                        get_string('addmorechoiceblanks', 'qtype_gapselectmath'),
                                        true);
    ...
    public function data_to_form($context, $fileoptions) {
        $gapselectmathoptions = array('answer' => array());
        if ($this->questionrec !== null) {
            $answers = array();
            foreach ($this->questionrec->options->answers as $answer) {
                $gapselectmathoptions['answer'][] = $answer->answer;
            }
        }
        return parent::data_to_form($context, $fileoptions) + $gapselectmathoptions;
    }
    ...
    public function validate() {
        ...
        foreach ($this->correctchoices as $correctchoice) {
            ...
            if (!isset($this->formdata->answer[$answerindex])) {
                $errors['questiontext'] = get_string('errormissingchoice', 'qtype_gapselectmath', $correctchoice);
            } else if ('' === trim($this->formdata->answer[$answerindex])) {
                $errors[$this->form_field_name("answer[{$answerindex}]")] =
                                                                get_string('errorblankchoice', 'qtype_gapselectmath', $correctchoice);
                $errors['questiontext'] = get_string('errormissingchoice', 'qtype_gapselectmath', $correctchoice);
            }
        }
