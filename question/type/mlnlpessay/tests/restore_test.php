...
class restore_test extends \restore_date_testcase {
    ...
    /**
     * Test missing qtype_mlnlpessay_options creation.
     *
     * Old backup files may contain mlnlpessays with no qtype_mlnlpessay_options record.
     * During restore, we add default options for any questions like that.
     * That is what is tested in this file.
     */
    public function test_restore_create_missing_qtype_mlnlpessay_options() {
        global $DB;

        // Create a course with one mlnlpessay question in its question bank.
        ...
        $mlnlpessay = $questiongenerator->create_question('mlnlpessay', null, array('category' => $category->id));

        // Remove the options record, which means that the backup will look like a backup made in an old Moodle.
        $DB->delete_records('qtype_mlnlpessay_options', ['questionid' => $mlnlpessay->id]);
        ...
        $newmlnlpessay = $DB->get_record_sql('SELECT q.*
                                              FROM {question} q
                                              JOIN {question_versions} qv ON qv.questionid = q.id
                                              JOIN {question_bank_entries} qbe ON qbe.id = qv.questionbankentryid
                                             WHERE qbe.questioncategoryid = ?
                                               AND q.qtype = ?', [$newcategory->id, 'mlnlpessay']);
        $this->assertTrue($DB->record_exists('qtype_mlnlpessay_options', ['questionid' => $newmlnlpessay->id]));
    }
}
