...
class question_test extends \advanced_testcase {
    public function test_get_question_summary() {
        $mlnlpessay = \test_question_maker::make_an_mlnlpessay_question();
        $mlnlpessay->questiontext = 'Hello <img src="http://example.com/globe.png" alt="world" />';
        $this->assertEquals('Hello [world]', $mlnlpessay->get_question_summary());
    }
    ...
    public function test_summarise_response(int $responserequired, int $attachmentsrequired,
        ...
        // Create the mlnlpessay question under test.
        $mlnlpessay = \test_question_maker::make_an_mlnlpessay_question();
        $mlnlpessay->start_attempt(new question_attempt_step(), 1);

        $mlnlpessay->responseformat = 'editor';
        $mlnlpessay->responserequired = $responserequired;
        $mlnlpessay->attachmentsrequired = $attachmentsrequired;
        ...
        $this->assertEquals($expected, $mlnlpessay->summarise_response(
    ...
    public function summarise_response_provider(): array {
        return [
            'text input required, not attachments required'  =>
                [1, 0, 'This is the text input for this mlnlpessay.', 0, 'This is the text input for this mlnlpessay.'],
            'Text input required, one attachments required, one uploaded'  =>
                [1, 1, 'This is the text input for this mlnlpessay.', 1, 'This is the text input for this mlnlpessay.Attachments: 0 (1 bytes)'],
    ...
    public function test_is_same_response() {
        ...
        $mlnlpessay->responsetemplate = '';

        $mlnlpessay->start_attempt(new question_attempt_step(), 1);

        $this->assertTrue($mlnlpessay->is_same_response(
                array(),
                array('answer' => '')));
        ...
        $this->assertFalse($mlnlpessay->is_same_response(
                array('answer' => 'Hello'),
                array()));
    ...
    public function test_is_same_response_with_template() {
        ...
        $mlnlpessay->responsetemplate = 'Once upon a time';
    ...
    public function test_is_complete_response() {
        ...
        $mlnlpessay->responserequired = 1;
        $mlnlpessay->attachmentsrequired = 0;
        $mlnlpessay->responseformat = 'editor';

        // The empty string should be considered an incomplete response, as should a lack of a response.
        $this->assertFalse($mlnlpessay->is_complete_response(array('answer' => '')));
        $this->assertFalse($mlnlpessay->is_complete_response(array()));

        // Any nonempty string should be considered a complete response.
        $this->assertTrue($mlnlpessay->is_complete_response(array('answer' => 'A student response.')));
        $this->assertTrue($mlnlpessay->is_complete_response(array('answer' => '0 times.')));
        $this->assertTrue($mlnlpessay->is_complete_response(array('answer' => '0')));

        // Test case for minimum and/or maximum word limit.
        $response = [];
        $response['answer'] = 'In this mlnlpessay, I will be testing a function called check_input_word_count().';

        $mlnlpessay->minwordlimit = 50; // The answer is shorter than the required minimum word limit.
        $this->assertFalse($mlnlpessay->is_complete_response($response));

        $mlnlpessay->minwordlimit = 10; // The  word count  meets the required minimum word limit.
        $this->assertTrue($mlnlpessay->is_complete_response($response));

        // The word count meets the required minimum  and maximum word limit.
        $mlnlpessay->minwordlimit = 10;
        $mlnlpessay->maxwordlimit = 15;
        $this->assertTrue($mlnlpessay->is_complete_response($response));

        // Unset the minwordlimit/maxwordlimit variables to avoid the extra check in is_complete_response() for further tests.
        $mlnlpessay->minwordlimit = null;
        $mlnlpessay->maxwordlimit = null;

        // Test the case where two files are required.
        $mlnlpessay->attachmentsrequired = 2;

        // Attaching less than two files should result in an incomplete response.
        $this->assertFalse($mlnlpessay->is_complete_response(array('answer' => 'A')));
        $this->assertFalse($mlnlpessay->is_complete_response(
        ...
        $this->assertTrue($mlnlpessay->is_complete_response(
        ...
        $mlnlpessay->reponserequired = 0;
        $mlnlpessay->responseformat = 'noinline';
        $mlnlpessay->attachmensrequired = 1;
    ...
    public function test_get_question_definition_for_external_rendering() {
        ...
        $mlnlpessay = \test_question_maker::make_an_mlnlpessay_question();
        $mlnlpessay->minwordlimit = 15;
        $mlnlpessay->start_attempt(new question_attempt_step(), 1);
        $qa = \test_question_maker::get_a_qa($mlnlpessay);
        $displayoptions = new question_display_options();

        $options = $mlnlpessay->get_question_definition_for_external_rendering($qa, $displayoptions);
        ...
        $this->assertEquals($mlnlpessay->minwordlimit, $options['minwordlimit']);
    ...
    public function test_get_validation_error(int $responserequired,
        ...
        $question = \test_question_maker::make_an_mlnlpessay_question();
    ...
    public function get_min_max_wordlimit_test_cases(): array {
        return [
            ...
            'text input required, min word limit not reached'  => [1, 15, 25,
                get_string('minwordlimitboundary', 'qtype_mlnlpessay', ['count' => 14, 'limit' => 15])],
            'text input required, max word limit is exceeded'  => [1, 5, 12,
                get_string('maxwordlimitboundary', 'qtype_mlnlpessay', ['count' => 14, 'limit' => 12])],
    ...
    public function get_word_count_message_for_review_test_cases() {
        return [
            ...
            'min and max, answer within range' =>
                    [10, 25, get_string('wordcount', 'qtype_mlnlpessay', 14)],
            'min and max, answer too short' =>
                    [15, 25, get_string('wordcounttoofew', 'qtype_mlnlpessay', ['count' => 14, 'limit' => 15])],
            'min and max, answer too long' =>
                    [5, 12, get_string('wordcounttoomuch', 'qtype_mlnlpessay', ['count' => 14, 'limit' => 12])],
            'min only, answer within range' =>
                    [14, null, get_string('wordcount', 'qtype_mlnlpessay', 14)],
            'min only, answer too short' =>
                    [15, null, get_string('wordcounttoofew', 'qtype_mlnlpessay', ['count' => 14, 'limit' => 15])],
            'max only, answer within range' =>
                    [null, 14, get_string('wordcount', 'qtype_mlnlpessay', 14)],
            'max only, answer too short' =>
                    [null, 13, get_string('wordcounttoomuch', 'qtype_mlnlpessay', ['count' => 14, 'limit' => 13])],
        ];
    }
    ...
    private function create_user_and_sample_attachments($numberofattachments = 4) {
        ...
        $helper = \test_question_maker::get_test_helper('mlnlpessay');
        $attachments = [];
