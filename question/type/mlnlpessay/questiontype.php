...
class qtype_mlnlpessay extends question_type {
    ...
    public function save_question_options($formdata) {
        ...
        $options = $DB->get_record('qtype_mlnlpessay_options', array('questionid' => $formdata->id));
        if (!$options) {
            $options = new stdClass();
            $options->questionid = $formdata->id;
            $options->categoriesweightteacher = isset($formdata->categoriesweightteacher)?$formdata->categoriesweightteacher:null;
            $options->categoriesweight = isset($formdata->categoriesweight)?$formdata->categoriesweight:'';
        ...
        $options->graderinfo = $this->import_or_save_files($formdata->graderinfo,
                $context, 'qtype_mlnlpessay', 'graderinfo', $formdata->id);
        ...
        if ($hascapedit) {
            foreach (json_decode($formdata->rubiccategoryfulltable) as $key => $value) {
                $choosen[$value->id] = [
                        'sortorder' => $key,
                        'id' => $value->id,
                        'name' => $value->name,
            ...
            if ($choosen) {
                ...
                $options->categoriesweightteacher = $formdata->categoriesweightteacher;
                $options->categoriesweight = $formdata->categoriesweight;
            }
        }
        $options->timemodified = time();
