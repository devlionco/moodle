 ...
 */
class file_storage implements \qtype_hvp_library\H5PFileStorage {
    ...
    private function getExportFile($filename) {
        ...
        $fs = get_file_storage();
        return $fs->get_file($context->id, 'qtype_hvp', 'exports', 0, '/', $filename);
    }
    ...
    public function cacheAssets(&$files, $key) {
        ...
        foreach ($files as $type => $assets) {
            ...
            foreach ($assets as $asset) {
                ...
                // Locate file.
                $file = $fs->get_file($context->id, 'qtype_hvp', $location[1], 0, $location[2], $location[3]);
    ...
    public function getCachedAssets($key) {
        ...
        $js = $fs->get_file($context->id, 'qtype_hvp', 'cachedassets', 0, '/', "{$key}.js");
        if ($js) {
        ...
        $css = $fs->get_file($context->id, 'qtype_hvp', 'cachedassets', 0, '/', "{$key}.css");
        if ($css) {
    ...
    public function deleteCachedAssets($keys) {
        ...
        foreach ($keys as $hash) {
            foreach (array('js', 'css') as $type) {
                $cachedasset = $fs->get_file($context->id, 'qtype_hvp', 'cachedassets', 0, '/', "{$hash}.{$type}");
                if ($cachedasset) {
                    $cachedasset->delete();
                }
            }
    ...
    public function cloneContentFile($file, $fromid, $tocontent) {
        ...
        if ($this->getFile('content', $sourceitemid, $file) !== false) {
    ...
    private static function exportFileTree($target, $contextid, $filearea, $filepath, $itemid = 0) {
        ...
        // Read source files.
        $fs = get_file_storage();
        $files = $fs->get_directory_files($contextid, 'qtype_hvp', $filearea, $itemid, $filepath, true);
    ...
    private static function deleteFileTree($contextid, $filearea, $filepath, $itemid = 0) {
        ...
        if ($filepath === '/') {
            ...
            $fs->delete_area_files($contextid, 'qtype_hvp', $filearea, $itemid);
            return;
        }
        ...
        // Remove root dir.
        $file = $fs->get_file($contextid, 'qtype_hvp', $filearea, $itemid, $filepath, '.');
        if ($file) {
            $file->delete();
        }
    }
    ...
    public static function fileExists($contextid, $filearea, $filepath, $filename) {
        // Check if file exists.
        $fs = get_file_storage();
        return ($fs->get_file($contextid, 'qtype_hvp', $filearea, 0, $filepath, $filename) !== false);
    }
    ...
    private static function moveFile($sourcefile, $contextid, $contentid) {
        ...
        $fileexists = $fs->file_exists($record['contextid'], 'qtype_hvp',
        ...
        if ($fileexists) {
            ...
            $file = $fs->get_file($record['contextid'], 'qtype_hvp',
                $record['filearea'], $record['itemid'], $record['filepath'],
                $record['filename']
            );
            if ($file) {
    ...
    public function getUpgradeScript($machinename, $majorversion, $minorversion) {
        ...
        $file = 'upgrades.js';
        if ($fs->get_file($context->id, 'qtype_hvp', $area, 0, $path, $file)) {
