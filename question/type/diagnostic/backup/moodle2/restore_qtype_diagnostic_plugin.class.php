 ...
 * needed to restore one diagnostic qtype plugin
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_qtype_diagnostic_plugin extends restore_qtype_plugin {
    ...
    protected function define_question_plugin_structure() {
        ...
        $elepath = $this->get_pathfor('/diagnostic');
    ...
    /**
     * Process the qtype/diagnostic element
     */
    public function process_diagnostic($data) {
        global $DB;
        ...
        // qtype_diagnostic_options too.
        if ($questioncreated) {
            $data->questionid = $newquestionid;

            // It is possible for old backup files to contain unique key violations.
            // We need to check to avoid that.
            if (!$DB->record_exists('qtype_diagnostic_options', array('questionid' => $data->questionid))) {
                $newitemid = $DB->insert_record('qtype_diagnostic_options', $data);
                $this->set_mapping('qtype_diagnostic_options', $oldid, $newitemid);
            }
        }
    }
     ...
     * recoded pointing to all the restored stuff for diagnostic questions
     *
    ...
    public static function define_decode_contents() {
        ...
        $contents[] = new restore_decode_content('qtype_diagnostic_options',
                $fields, 'qtype_diagnostic_options');
