 ...
 * @copyright  2018 Nadav Kavalerchik (modified to support question context)
 * @copyright  2015 Damyon Wiese (original code)
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class question_competency extends \core\persistent {
    ...
    public static function get_ruleoutcome_name($ruleoutcome) {
        ...
        // TODO: use unique question competencies (when integrated into core Moodle)
        //return new lang_string('questioncompetencyoutcome_' . $strname, 'core_competency');
        // For now, use coursemodulecompetencyoutcome. (nadavkav)
...
//todo: fix me
    ...
    public static function update_question_competencies($question, $fromform) {
        ...
        foreach ($removed as $removedid) {
            remove_competency_from_question($question->id, $removedid);
        }
        foreach ($added as $addedid) {
            add_competency_to_question($question->id, $addedid);
        }

        if (isset($fromform->competency_rule)) {
            // Now update the rules for each course_module_competency.
            $current = list_question_competencies_in_question($question->id);
            foreach ($current as $questioncompetency) {
                set_question_competency_ruleoutcome($questioncompetency, $fromform->competency_rule);
            }
        }
    }
