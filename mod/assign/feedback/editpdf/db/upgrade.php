...
function xmldb_assignfeedback_editpdf_upgrade($oldversion) {
    ...
    if ($oldversion < 2022032100) {
        ...
        // Editpdf savepoint reached.
        upgrade_plugin_savepoint(true, 2022032100, 'assignfeedback', 'editpdf');
    }

    if ($oldversion < 2022042600) {
        $table = new xmldb_table('assignfeedback_editpdf_absq');
        $field = new \xmldb_field('sequence', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'points');

        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        upgrade_plugin_savepoint(true, 2022042600, 'assignfeedback', 'editpdf');
    }

    if ($oldversion < 2022050200) {
        $table = new xmldb_table('assignfeedback_editpdf_absq');
        $field = new \xmldb_field('questionid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'sequence');
