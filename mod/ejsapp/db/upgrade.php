...
function xmldb_ejsapp_upgrade($oldversion) {
    ...
    if ($oldversion < '2019080400') {
        ...
        try {
            ...
            mtrace("No need to drop field: {$field->getName()} from table: {$table->getName()} \n");
            //throw $ex;
        }
    ...
    if ($oldversion < '2019081103') {
        ...
        $dbman = $DB->get_manager();
        $table = new xmldb_table('ejsapp');
        $field = new xmldb_field('applet_name', XMLDB_TYPE_TEXT, '64', null, null, null, null, 'timemodified'); //, 'class_file');
        try {
