...
function evoting_update_instance($evoting)
{
    ...
    if (isset($evoting->questionname[$cptQuestion]['text'])) {
        ...
        while (trim($evoting->questionname[$cptQuestion]['text']) != "") {
            ...
            // Remove duplicated 'number' error while reorder questions.
            $exist = $DB->get_record_select("evoting_questions", "number = :number AND evotingid = :evotingid AND id <> :id", array('number' => $question->number, 'evotingid' => $question->evotingid, 'id' => $question->id));
            if ($exist) {
                $max = $DB->get_record_sql("SELECT MAX(number) as number FROM {evoting_questions} WHERE evotingid = ?",
                    array($question->evotingid));
                $exist->number = $max->number + 1;
                $DB->update_record("evoting_questions", $exist);
            }
