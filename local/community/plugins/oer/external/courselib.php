    ...
    public static function get_course_blocks($presets) {
        ...
        foreach ($data as $item) {
            if ($item->area == 'sidemenu') {
                switch ($item->action) {
                    case 'category':
                        $res = $DB->get_records('community_oer_course', ['catid' => $item->value]);
                        ...
                        break;
                    case 'course':
                        $res = $DB->get_records('community_oer_course', ['courseid' => $item->value]);
                        list($activitytotal, $questiontotal, $sequencetotal, $coursetotal) =
                                \community_oer\main_oer::total_elements_of_plugins($item->action, $item->value, $data);
                        break;
                    case 'section':
                        $res = $DB->get_records('community_oer_course', ['sectionid' => $item->value]);
                        list($activitytotal, $questiontotal, $sequencetotal, $coursetotal) =
                                \community_oer\main_oer::total_elements_of_plugins($item->action, $item->value, $data);
                        break;
                }
        ...
        $course = new \community_oer\course_oer;
        $cache = $course->get_courses_from_cache();
