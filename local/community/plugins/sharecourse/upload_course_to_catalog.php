 ...
 * @package   community_sharecourse
 */
class upload_course_to_catalog extends moodleform {
    ...
    public function definition() {
        ...
        // Build hidden inputs.
        $html = '<input type="hidden" name="typeshare" value="' . $default['typeshare'] . '">';
        $mform->addElement('html', $html);

        $sharecourse = new \community_sharecourse\sharecourse();
        $sharecourse->prepare_active_fields();

        foreach ($sharecourse->get_required_fields() as $item) {
            $this->build_element($item, $default);
        }

        $shownonrequieredfields = get_config('community_sharecourse', 'shownonrequieredfields');
        $notrequiredfields = $sharecourse->get_not_required_fields();
        if (!empty($notrequiredfields)) {
            if ($shownonrequieredfields == 1) {
                $html = '<button class="btn btn-primary mb-2" type="button" data-toggle="collapse" data-target="#collapseNotRequired"
                    ...
                    ' . get_string('advanced_catalog_options', 'community_sharecourse') . '                
                </button>
        ...
        switch ($item->datatype) {
            ...
            case 'selectcourses':
                if (isset($default[$item->shortname])) {
                    foreach ($item->hierarchy as $key1 => $cat) {
                        if (isset($cat['children'])) {
                            foreach ($cat['children'] as $key2 => $course) {
                                if (in_array($course['courseid'], $default[$item->shortname])) {
                                    $item->hierarchy[$key1]['children'][$key2]['checked'] = true;
                                }
                            }
                        }
                    }
                ...
                $item->data = json_encode($item->hierarchy, JSON_UNESCAPED_UNICODE);

                $html = $OUTPUT->render_from_template('community_sharecourse/elements/select-course-block', $item);
                $mform->addElement('html', $html);
                break;
            ...
            case 'durationactivity':
                ...
                $html = $OUTPUT->render_from_template('community_sharecourse/elements/radio-duration', $item);
                $mform->addElement('html', $html);
                break;
            ...
            case 'levelactivity':
                ...
                $html = $OUTPUT->render_from_template('community_sharecourse/elements/radio-buttons-row-difficalty', $item);
                $mform->addElement('html', $html);
                break;
            ...
            case 'originality':
                if (isset($default[$item->shortname]) &&
                    ...
                    $item->checkbox_no = true;
                    $item->checkbox_yes = false;
                    $item->textarea_value = '';
                }
                $html = $OUTPUT->render_from_template('community_sharecourse/elements/radio-buttons-column', $item);
                $mform->addElement('html', $html);
                break;

            case 'tags':
                $html = $OUTPUT->render_from_template('community_sharecourse/elements/tags', $item);
                $mform->addElement('html', $html);
                break;

            // Standart types.
            case 'fileupload':
                $html = $OUTPUT->render_from_template('community_sharecourse/elements/form-item', $item);
            ...
            case 'text':
                ...
                $html = $OUTPUT->render_from_template('community_sharecourse/elements/input', $item);
                $mform->addElement('html', $html);
                break;
            ...
            case 'checkbox':
                ...
                $html = $OUTPUT->render_from_template('community_sharecourse/elements/checkbox', $item);
                $mform->addElement('html', $html);
                break;
        }
    }
