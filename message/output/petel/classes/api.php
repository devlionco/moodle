...
/**
 * Contains class used to return information to display for the message petel.
 *
...
/**
 * Class used to return information to display for the message petel.
 *
 * @copyright   2019 Devlion <info@devlion.co>
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class api {
    /**
     * Get petel notifications for the specified users. Nothing is returned if notifications are disabled.
     *
     ...
     * @since 3.2
     */
    public static function get_petel_notifications($useridto = 0, $sort = 'DESC', $limit = 0, $offset = 0) {
        global $DB, $USER;
        ...
        $sql = "SELECT n.id, n.useridfrom, n.useridto,
                       ...
                       n.eventtype, n.timeread, n.customdata, mpn.popupremoved
                  FROM {notifications} n
                  LEFT JOIN {message_petel_notifications} mpn ON(mpn.notificationid = n.id)
                 WHERE n.id IN (SELECT notificationid FROM {message_petel_notifications})
                   AND n.useridto = ?
     ...
     * @since 3.2
     */
    public static function count_unread_petel_notifications($useridto = 0) {
        global $USER, $DB;
        ...
        return $DB->count_records_sql(
                "SELECT count(id)
               FROM {notifications}
              WHERE id IN (SELECT notificationid FROM {message_petel_notifications})
                AND useridto = ?
