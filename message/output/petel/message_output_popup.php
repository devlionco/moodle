...
/**
 * The petel message processor
 *
 * @package     message_petel
 * @copyright   2019 Devlion <info@devlion.co>
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class message_output_petel extends message_output {

    /**
     * Adds notifications to the 'message_petel_notifications' table if applicable.
     *
     * The reason for this is because we may not want to show all notifications in the notification popover. This
     * can happen if the petel processor was disabled when the notification was sent. If the processor is disabled this
     * function is never called so the notification will never be added to the 'message_petel_notifications' table.
    ...
    public function send_message($eventdata) {
        ...
        // Prevent users from getting petel notifications from themselves (happens with forum notifications).
        if ($eventdata->userfrom->id != $eventdata->userto->id && $eventdata->notification) {
            if (!$DB->record_exists('message_petel_notifications', ['notificationid' => $eventdata->savedmessageid])) {
                $record = new stdClass();
                $record->notificationid = $eventdata->savedmessageid;

                $DB->insert_record('message_petel_notifications', $record);
            }
        }
