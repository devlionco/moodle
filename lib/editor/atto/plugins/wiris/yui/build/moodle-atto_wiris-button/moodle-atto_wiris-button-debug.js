    ...
    initializer: function(config) {
        ...
        Y.Get.js(M.cfg.wwwroot + '/lib/editor/atto/plugins/wiris/core.js?v=' + config.version, function(err) {
            if (err) {
                ...
                if($('.htmleditorbutton')[0]) {
                    $('.atto_wiris_button_wiris_editor').on('click', () => {
                        const wirisLoad = setInterval(() => {
                            if($('.wrs_focusElement')[0]) {
                                $('.wrs_focusElement').on("blur", () => {
                                    $('.wrs_focusElement').focus()
                                })
                                $('.wrs_focusElement')[0].focus()
                                $('.wrs_focusElement')[0].autofocus = true;
                                $(".wrs_modal_dialogContainer").css("z-index", "2147483648");
                                clearInterval(wirisLoad)
                            }
                        },200)
