 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_picturelink_renderer extends core_courseformat\output\section_renderer {
    // TODO.
    ...
    public function activitiesselection($modinfo, $course) {
        ...
        $o .= html_writer::tag('span', '', array('class' => 'select-icon d-none d-md-inline'));
    ...
    public function picturelink_get_cms($course, $modinfo) {
        ...
        if (has_capability('moodle/course:update', $context)) {
            ...
            $o .= html_writer::end_tag('div');
            $o .= html_writer::tag('div', get_string('moveitems', 'format_picturelink'), array('class' => 'd-none d-md-inline'));
        ...
        foreach ($modinfo->cms as $cm) {
            ...
            if($availableclass == '' and $corevisibleclass == '' and $activeclass == '' ){
                $defaultstatclass = ' defaultstat';
            }

            $content = $course->showactivityname ? '<span class = "picturelink_itemcontent">'.$cm->name.'</span>' : '';
            $o .= html_writer::link($link, $content, array(
                        'class' => 'picturelink_item drag' . $activeclass . $corevisibleclass . $availableclass . $newclass . $defaultstatclass,
                        'data-id' => $cm->id,
        ...
        foreach ($modinfo->sections as $section => $scms) {
            ...
            // *** from here hanna 3/3/2020
            $a = $sinfo->uservisible;
            $b = (isset($visibleitems[$sid])) ? $visibleitems[$sid] : 0;
            if ($a) {
                $visibl = isset($b) ? $b : 0;
            } else {
                $visibl = 0;
            }
            //  **** till here check if section is accessable   hanna  was 'data-visibility' => isset($visibleitems[$sid]) ? $visibleitems[$sid] : 0,
        ...
        // TODO: use pics from course format settings:defaultstat, completed, locked, hiden  hanna 24/6/2020
        if ($status_defaultstat_imgurl = $this->picturelink_get_status_image($course, 'picturelinkimage_defaultstat')) {
            $o .= html_writer::tag('style','
                    .format-picturelink .picturelink .defaultstat {
                        background: url('.$status_defaultstat_imgurl.') no-repeat center;
                        }
                    ');
        }
        if ($status_completed_imgurl = $this->picturelink_get_status_image($course,'picturelinkimage_completed')) {
            $o .= html_writer::tag('style','
                    .format-picturelink .picturelink .completed {
                        background: url('.$status_completed_imgurl.') no-repeat center;
                        }
                    ');
        }
        if ($status_locked_imgurl = $this->picturelink_get_status_image($course, 'picturelinkimage_locked')) {
            $o .= html_writer::tag('style','
                    .format-picturelink .picturelink .p_locked {
                        background: url('.$status_locked_imgurl.') no-repeat center;
                        }
                    ');
        }
        if ($status_hiden_imgurl = $this->picturelink_get_status_image($course, 'picturelinkimage_hiden')) {
            $o .= html_writer::tag('style','
                    .format-picturelink .picturelink .p_hide {
                        background: url('.$status_hiden_imgurl.') no-repeat center;
                        }
                    ');
        }
    ...
    /**
     * Function gets image for picturelink status::completed  // hanna 24/6/2020
     * @param $course
     * @return $picturelinkimage - link to status::completed  image
     */
    private function picturelink_get_status_image($course,$statusimage) {  // todo add parameter for status field?
        $context = context_course::instance($course->id);
        $fs = get_file_storage();
     //   if ($files = $fs->get_area_files($context->id, 'format_picturelink', 'picturelinkimage_completed', $course->id)) {
        if ($files = $fs->get_area_files($context->id, 'format_picturelink', $statusimage, $course->id)) {
            foreach ($files as $file) {
                if ($file->get_filename() != '.') {
                    $picturelinkstatusimage = moodle_url::make_pluginfile_url(
                        $file->get_contextid(),
                        $file->get_component(),
                        $file->get_filearea(),
                        $file->get_itemid(),
     ...
     //   $status_completed_url = $this->courserenderer->image_url('status_completed', 'format_picturelink');
        switch($statusimage){
            case 'picturelinkimage_defaultstat':
                $status_url = $this->courserenderer->image_url('status_defaultstat', 'format_picturelink');
                break;
            case 'picturelinkimage_completed':
                $status_url = $this->courserenderer->image_url('status_completed', 'format_picturelink');
                break;
            case 'picturelinkimage_locked':
                $status_url = $this->courserenderer->image_url('status_locked', 'format_picturelink');
                break;
            case 'picturelinkimage_hiden':
                $status_url = $this->courserenderer->image_url('status_hiden', 'format_picturelink');
                break;
        }
     //   $status_image_url = $this->courserenderer->image_url('status_completed', 'format_picturelink');
    //    $picturelinkstatusimage = isset($picturelinkstatusimage) ? $picturelinkstatusimage : $status_completed_url;

     //   $picturelinkstatusimage = isset($picturelinkstatusimage) ? $picturelinkstatusimage : $status_url;
         $picturelinkstatusimage = isset($picturelinkstatusimage) ? $picturelinkstatusimage : null;

        return $picturelinkstatusimage;
    }
